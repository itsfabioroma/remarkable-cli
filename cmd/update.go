...
Your binary is swapped in place with no manual steps.`,
  ...
  remarkable update --check   # check only, no install`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		defer cancel()
		latestSHA, latestDate, err := update.ForceCheck(ctx)
		if err != nil {
		...
		if flagUpdateCheck {
			switch {
			case curSHA == "":
				fmt.Printf("latest on main: %s (%s)\n", short(latestSHA), latestDate[:10])
			case remoteNewer:
				fmt.Printf("update available: %s → %s (%s)\n", short(curSHA), short(latestSHA), latestDate[:10])
			default:
				fmt.Printf("you're ahead of main: local %s, remote %s\n", short(curSHA), short(latestSHA))
			}
			return nil
		}

		if !remoteNewer && curSHA != "" {
			fmt.Printf("you're ahead of main — nothing to update (local %s, remote %s)\n", short(curSHA), short(latestSHA))
			return nil
		}
